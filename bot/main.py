...
def menu_status(call):
    uid = call.from_user.id
    if not has_access(uid):
        bot.answer_callback_query(call.id, "❌ Accès refusé", show_alert=True)
        return
    
    try:
        stats = system_core.get_system_stats()
        uptime = system_core.get_uptime()
        info = system_core.get_server_info()
        
        text = f"""
<b>📊 STATUT DU SERVEUR</b>

<b>Serveur:</b>
  • <code>{info.get('hostname', 'N/A')}</code>
  • IP: <code>{info.get('ip', 'N/A')}</code>

<b>Ressources:</b>
  • CPU: <b>{stats.get('cpu', 'N/A')}%</b>
  • Mémoire: <b>{stats.get('memory_used', 0)}/{stats.get('memory_total', 0)} GB</b>
  • Disque: <b>{stats.get('disk_used', 0)}/{stats.get('disk_total', 0)} GB</b>
...
<b>Uptime:</b> {uptime}
        ...
        _show_submenu(call, text.strip(), markup)
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Erreur: {str(e)}", show_alert=True)

@bot.callback_query_handler(func=lambda call: call.data == "action_home")
def action_home(call):
    uid = call.from_user.id
    try:
        bot.delete_message(call.message.chat.id, call.message.message_id)
    except:
...
def menu_handler(call):
    ...
    proto = call.data.replace("menu_", "")
    text = f"<b>🔌 Menu {proto.upper()}</b>\n\nSélectionnez une action:"
    markup = protocol_menu_keyboard(proto, uid)
