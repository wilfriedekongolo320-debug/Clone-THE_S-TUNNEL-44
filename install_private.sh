...
echo "   1 = Dépôt public (recommandé)"
echo "   2 = Dépôt privé avec token"
echo ""
...
if [ "$choice" == "1" ]; then
    ...
    echo -e "${RED}❌ Choix invalide! Aborted.${NC}"
    exit 1
fi
...
if [ -z "$GITHUB_TOKEN" ]; then
    # Dépôt public - pas d'authentification
    if ! NEXUS_SCRIPT=$(curl -s "${REPO_URL}/nexus.sh"); then
        echo -e "${RED}❌ Erreur: Impossible de télécharger nexus.sh${NC}"
        exit 1
    fi
else
    # Dépôt privé - avec authentification
...
if [ -z "$GITHUB_TOKEN" ]; then
    bash "$INSTALLER_SCRIPT" "$REPO_URL" ""
else
    bash "$INSTALLER_SCRIPT" "$REPO_URL" "$GITHUB_TOKEN"
fi
